...
func init(ida :int, 반지름a :float, 깊이a :float, 
	...
	$"돌림/BarTree2".init_common_params(반지름*0.5, 깊이, 반지름*0.05, 256, rot, deg_to_rad(선택각도), 1.0, false 
		).init_with_color(장식색, 원판색)
	$"돌림/BarTree2".position.y = 깊이/2
	$"돌림/BarTree3".init_common_params(반지름*0.5, 깊이, 반지름*0.05, 256, rot, deg_to_rad(선택각도+180), 1.0, false 
		).init_with_color(장식색.inverted(), 원판색.inverted())
	$"돌림/BarTree3".position.y = 깊이/2
...
func 선택된칸얻기() -> 칸:
	var 각도 = rad_to_deg($"돌림/원판".rotation.y) - 선택각도
	if 칸들.size() == 0 :
		return null
	while 각도 < 0:
		각도 += 360
